...
//
// Stored as a small JSON sidecar per reference (base64 + mime) under
// .data/receipts/ — same local-file-simulated pattern as the rest of the
// store. Not meant for high volume; fine for a single venue's traffic.
...
export function saveDepositReceipt(
  reference: string,
  mimeType: string,
  base64Data: string,
): void {
...
export function getDepositReceipt(
  reference: string,
...
export function hasDepositReceipt(reference: string): boolean {
